...
use algebra::{Field, PairingEngine as ConstraintEngine};
...
/// MulCircuit is a circuit that checks whether, for a given `a[0]`, `a[1]`, and `c`,
/// the prover knows `b` such that `(2 * a[0] + a[1]) * b = c`.
...
impl<E: ConstraintEngine> MulCircuit<E> {
    ...
    pub fn with_values(a: [E::Fr; 2], b: E::Fr, c: E::Fr) -> Self {
...
impl<E: ConstraintEngine> Circuit<E> for MulCircuit<E> {
    fn synthesize<CS: ConstraintSystem<E>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
        ...
        let a_0: Variable = cs.alloc_input(|| "a", || self.a.ok_or(SynthesisError::AssignmentMissing).map(|a| a[0]))?;
        let a_1: Variable = cs.alloc_input(|| "a", || self.a.ok_or(SynthesisError::AssignmentMissing).map(|a| a[1]))?;
